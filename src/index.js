...
Cypress.Commands.add('timeMark', { prevSubject: 'optional' }, (subject, name) => {
...
Cypress.Commands.add('timeSince', { prevSubject: 'optional' }, (subject, name, label, timeLimit, throwError) => {
	if (typeof name !== 'string') {
		throw new Error('Expected a time mark name');
	}
	if (!name) {
	...
	if (label) {
		if (timeLimit && elapsed > timeLimit) {
			cy.log(`🆘 ${formatted} ${label} since **${name}** (above the time limit ${timeLimit}ms)`);
			if (throwError) {
			...
			//return object with markerTitle and elapsed time
			return cy.wrap({ marker: name, time: formatted });
		}
	} else {
		if (timeLimit && elapsed > timeLimit) {
			cy.log(`🆘 ${formatted} since **${name}** (above the time limit ${timeLimit}ms)`);
			if (throwError) {
				cy.log('**cypress-time-marks throwing an error**').then(() => {
